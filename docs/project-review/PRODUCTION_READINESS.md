...
- [ ] **DAT-1** — client data and credentials out of cloud sync; dumps relocated. **Local half done 2026-08-27:** `data_dir` moved to `C:\Users\i81129\odoo-data`, the two `pg_dump` images to `C:\Users\i81129\odoo-backups`, 154 session files purged, `.odoo_data` gone from the tree; filestore proven intact (84 store-backed rows, 84 readable, 0 empty, identical to the pre-move baseline). **The gate stays shut** because relocation does not retract what the tenant already holds — OneDrive cloud copies, version history and the retention/recycle bin may still contain both dumps, the sessions and `odoo.conf`. Tick this only once that cloud-side purge is confirmed, which is a OneDrive-web or Verisk-IT action. `.git` (340 MB) is also still in the sync root — the *integrity* half of the finding
