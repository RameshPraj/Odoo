...
> 2026-08-30 the *derived* figures are **66 RESOLVED, 6 PARTIAL (ACC-6, CI-1, CI-2, DAT-1, SEC-8, TST-7), 49 OPEN**.
...
**RESOLVED AS SCOPED 2026-08-30 — the concern moved module; see below** · Testing · `l10n_np_bs/static/src/`
...
**Note 2026-08-30 — this finding's subject no longer exists.** `l10n_np_bs` has **no `static/`
directory at all** and its manifest is 28 lines with no `assets` key, so there is no untested
JavaScript there and no dangling bundle glob. The module became a compatibility shim and the code
moved to `nepali_calendar_core`, which now carries **7 source files / 1,261 lines against 2 hoot
...
| **BS-3** | **RESOLVED — stale; the cited file does not exist.** Verified 2026-08-31: `bs_accounting_dates.py` is gone, the same dead reference as UPG-7 and BS-7. Bikram Sambat became global coverage in `nepali_calendar_core` rather than a per-model allowlist, which dissolved this rather than fixing it | Bikram Sambat | `bs_accounting_dates.py:57-60`; `account_move_views.xml:365` | Search and group-by buckets stay Gregorian | A BS user filtering "August 2026" gets Bhadra 16 – Ashoj 15, straddling two BS months. **The most consequential functional gap** — BS presentation stops where periodic reporting begins | BS-aware period filter and group-by | L |
...
| **BS-7** | **RESOLVED — stale; same dead file as BS-3 and UPG-7.** Verified 2026-08-31: `bs_accounting_dates.py` no longer exists | Bikram Sambat | `bs_accounting_dates.py:70-81` | Name-based patching walks nested subviews of **other** models | Latent today; a future non-date field named `date` in a subview gets `bs_date` | Check `_fields[name].type` | XS |
...
| **ACC-4** | **CONFIRMED — SME decision, recorded 2026-08-31; stays open.** Verified: `account.fiscal.position-np.csv` gives the Export position `auto_apply=1` with an **empty `country_id`**, so it applies to every partner the Domestic position does not match. Odoo fiscal positions do not distinguish customers from vendors, so this also zero-rates **foreign vendor bills**, which reverse charge should tax. Three remedies, and choosing between them is Nepali VAT policy (**SME item 6**), not engineering: (a) target a country or country group so it applies only where intended; (b) set `vat_required` so it applies only to VAT-registered partners; (c) drop `auto_apply` and let staff apply it deliberately. Not guessed at — the mechanism is recorded so the SME chooses from evidence | Accounting | `account.fiscal.position-np.csv:3` | Export FP `auto_apply=1` with no country | Also zero-rates foreign **vendors** — wrong under reverse charge | SME decision | S |
| **ACC-5** | **CONFIRMED — SME decision, recorded 2026-08-31; stays open.** Verified: `l10n_np_200903 \"Tax Receivable\"` is typed `asset_current` at code 200903, between `200902 VAT Payable to IRD` and the rest of an all-liability 2xxxxx block, so it reads as an asset filed among liabilities on a code-ordered trial balance. Two defensible readings, and the register should not pick one: it is **misfiled** and belongs in the 1xxxxx asset block, or it is **deliberately grouped** with the VAT settlement accounts by function, which is a real charting style. Chart structure is **SME item 1**. Renumbering also has a cost worth stating: the code is what orders every historical report, so changing it on an account that already carries postings reorders the past as well as the future | Accounting | `l10n_np/data/template/account.account-np.csv:24` | `Tax Receivable` typed `asset_current` but sits in the 2xxxxx liability block | Reads wrong on a code-ordered trial balance | Re-code or re-type | XS |
| **ACC-6** | **PARTLY RESOLVED 2026-08-31 — the company-scoping half is fixed; the year half is SME-gated.** **Fixed:** the sequence shipped with `company_id = False`, and `next_by_code` matches `company_id in (False, env.company.id)`, so every company drew from one row — issuing in one advanced the numbering of all. Now one series per company, resolved and lazily created by `_certificate_sequence()` keyed on **the certificate's own company**, not `env.company`: those differ exactly when a multi-company user issues for another company, which is when a wrong number is least likely to be noticed. `migrations/19.0.1.3.0` converts the existing row **in place** rather than creating a new one — verified afterwards: still one sequence, now owned by My Company, counter intact at next=320. A replacement would have restarted at 1 and reissued numbers. **Not fixed, deliberately:** the prefix interpolates `%(range_year)s`, which is the **Gregorian** year — `range_date` defaults to now (`odoo/addons/base/models/ir_sequence.py:220-227`) — so a certificate issued today reads `TDS/2026/…` when the Nepali fiscal year is **2083/84**. That changes what a statutory document's identifier says, and the certificate format is **SME item 3**. Incidental observation worth keeping: the counter stood at 319 with **zero** certificates ever issued, because `ir.sequence` is backed by a non-transactional PostgreSQL sequence and every rolled-back test still consumes a number | Accounting | `l10n_np_tds/data/ir_sequence_data.xml:6,9` | TDS sequence uses the **Gregorian** year and is **company-global** (`company_id=False`) | All tenants share one certificate series; the year rolls mid-Nepali-FY | Per-company sequence, BS year | S |
...
| **UPG-3** | **RESOLVED 2026-08-30** — the xpath now anchors to `//app[@name='account']` with `position=\"inside\"` instead of `//block[@id='analytic']` with `position=\"before\"`. The block it hung off is core's (`account/views/res_config_settings_views.xml:354`) and sits in the arch Odoo churns most; an xpath that misses does not degrade, it raises ParseError, and a ParseError in a view the module installs means **the module fails to install**. The app is the container that block already lives in (`:23-388`), so the setting still renders in the same place under the same visibility; only the anchor changes | Upgrade | `res_config_settings_views.xml:10` | xpath `//block[@id='analytic']` `position="before"` — sibling-relative into the most-churned arch in Odoo | A rename → `ParseError` → **the module fails to install** | Target the ancestor with `position="inside"` | S |
| **UPG-4** | **RESOLVED 2026-08-30** — renamed `account.lock.dates` to `l10n_np.account.lock.dates`. `_name` is not a reservation: had Odoo 20 introduced that name, this class would have stopped *defining* a model and silently started *extending* Odoo's, merging five lock-date fields into it during an upgrade. Every other locally invented model here was already prefixed; this was the sole exception. Verified after the rename rather than assumed: the new model is registered, the **old `ir.model` row is gone** so the namespace is genuinely released, and the ACL and act_window both point at the new name. Residue, recorded not fixed: the empty `account_lock_dates` transient table remains, because Odoo never drops tables and a destructive statement is not worth issuing for a vacuumed table with no rows, no model and no foreign keys | Upgrade | `wizard/account_lock_dates.py:18` | New model named `account.lock.dates` — inside core's namespace | If Odoo 20 adds that name, this silently *extends* it and the fields collide | Rename `l10n_np.account.lock.dates` | XS |
...
| **COD-1** | **RESOLVED 2026-08-30** — every monetary comparison in `test_loan.py` now goes through `assertMoneyEqual`, which calls `currency.compare_amounts` so the tolerance comes from the currency's own precision rather than from a `places=` picked in the test. It generalises the single `currency_id.round()` that was the only mitigation. Proved to bite rather than merely to pass: measured against NPR it absorbs 1e-7 accumulation noise and **fails on one paisa** | Tests | `test_loan.py` (10 sites) | Money compared with `assertEqual`; the `currency.round()` mitigation appears in exactly one test | Brittle across rate/term combinations | Round consistently | S |
...
| **COD-3** | **RESOLVED 2026-08-30** — six empty untracked directories removed, so a fresh clone and this working copy now agree. Four were `l10n_np_bs/static/{description,src,tests}` and `views`, left behind when that module became a shim; the others were `l10n_np_fiscal_year/models` and `nepali_calendar_core/migrations`. The finding's second clause, 'the declared JS test bundle location does not exist on a fresh clone', is **stale**: that manifest declares no bundle any more (see PKG-1). Safe by construction — every directory was both empty and untracked, so nothing left the repository that was ever in it | Packaging | 3 dirs | Empty package directories on disk but untracked | `git clone` ≠ working copy; the declared JS test bundle location does not exist on a fresh clone | Delete or `.gitkeep` | XS |
| **SEC-4** | **RESOLVED 2026-08-30** — `_set_default_wizard_values` is overridden in `account_reports_interactive` to write `user_id=self.env.uid` instead of `user_id=False`, so a remembered label limit belongs to whoever chose it rather than to the whole company. A local `_inherit` on the abstract model covers all six wizards at once; the vendored file is untouched per VENDORED.md. **Stopping the write did not undo it**: four company-wide rows were already in the live database, one per report anyone had exported, and `migrations/19.0.1.2.1` deletes them (verified: 4 removed, 0 remain). Deleted rather than reassigned — who set them was never recorded, which is the bug | Security | `abstract_wizard.py:67-75` | `sudo()` writes a global `ir.default` on every Export click; wizards open to `base.group_user` | Any employee writes a company-wide default | Local override | S |
| **SEC-5** | **RESOLVED 2026-08-30** — both grants narrowed by updating the vendored rows by fully-qualified xmlid from local modules, never by editing the CSVs (VENDORED.md). `account.age.report.configuration` and its lines: `base.group_user` drops to read-only, with Accountant (1,1,1,0) and Adviser (1,1,1,1) rows added alongside — necessary, because the vendored CSV had exactly one row per model, so narrowing alone would have left the ageing buckets editable by nobody. `crossovered.budget.lines`: the `access_budget` row — named 'manager' yet granted to `base.group_user`, and redundant beside the accountant row above it — drops to read-only. Read is kept in both cases deliberately: the aged report resolves the configuration while rendering, and employee visibility of a budget is plausibly intended upstream. Asserted against a real employee's effective rights in `test_oca_acl_overrides.py`, because ACLs are a union and a narrowing that failed to apply would look identical in the source | Security | OCA ACL CSVs | `base.group_user` CRUD on budget lines and the persistent age-report configuration | Non-accounting staff alter aged-balance bucketing | Tighten locally | S |
| **SEC-7** | **RESOLVED 2026-08-30** — `--modules` values now go through `_module_subdir()`, which resolves the path and refuses anything whose `os.path.commonpath` with the addons root is not the addons root. `os.path.join` treats `..` and absolute components as instructions rather than data, and step 4 then calls `os.makedirs` and `shutil.copyfile` on the result, so a mistyped value wrote `ne.po` wherever it pointed. Operator-run, so bounded — but bounded is not guarded, and the check costs one comparison. Proved by firing it: legitimate allowed; parent traversal, deep traversal, an absolute path and traversal-after-a-valid-prefix all refused. `commonpath` rather than `startswith`, and that distinction was measured too — `startswith` accepts a sibling directory named `<addons>-evil` | Security | `l10n_ne/apply.py:88,112,148,179-181` | `--modules` unvalidated into `os.path.join`; no containment check | Operator-run, so bounded — an unguarded footgun in a routine tool | `os.path.commonpath` | XS |
| **SEC-8** | **PARTLY RESOLVED 2026-08-30 — two of three sub-findings; the third is deliberately left.** **(1) Bare `@route()` — FIXED.** Odoo merges an overriding controller's routing over the parent's (`odoo/http.py:933`), so a bare decorator inherits an auth level rather than choosing one. Core says `auth='user'` today (`web/controllers/report.py:26,92`), so this was never an open door — it was an unstated dependency that would have followed a future upstream relaxation with no diff and no warning. `account_reports_interactive/controllers/main.py` restates `auth=\"user\"` and *only* auth, letting paths, `type`, `website` and `readonly` keep merging from the parent. Asserted behaviourally: an anonymous caller must be redirected to `/web/login`, the only outcome distinguishing refusal for *who you are* from refusal for *what you sent* — the first draft asserted merely 'not 200', which a missing-parameter error also satisfies. **(2) Client context merged into rendering — mitigated by the ORM, not by us.** The register proposed an allowlist; measured first, the one key with teeth, `allowed_company_ids`, is validated by `Environment.company`/`.companies`, which raise AccessError on unauthorized companies (`odoo/orm/environments.py:241,271`). An allowlist would be speculative hardening against a mostly-closed hole while risking silent breakage of any report needing a key nobody enumerated. Residual: content-shaping keys such as `active_test` still pass through, within data the caller may already read. **(3) `_serialize_exception` returned — NOT fixed, on purpose.** Its `debug` key is `traceback.format_exc()`: server paths, html-escaped so not XSS but disclosure. The `except` body is inseparable from the method, so every local fix means copying it — and `report_xlsx` is **AGPL-3** while `account_reports_interactive` is **LGPL-3**, so the copy would put AGPL code in an LGPL-declared module, the exact mixing **LIC-1** tracks. Belongs upstream as an OCA patch; this part stays open | Security | `report_xlsx/controllers/main.py:27,36-41,100-104` | Bare `@route()` inherits auth invisibly; client context merged into rendering; `_serialize_exception` returned | Auth tracks upstream silently; info disclosure (escaped, not XSS) | Restate `auth=`; allowlist context | S |
...
| **OPS-4** | **RESOLVED — already correct; verified 2026-08-30.** The finding's own fix note said 'restore on Linux (already correct there)', and it is: `deploy/odoo.conf.linux.example:52-53` ships `limit_time_real = 120` and `limit_time_real_cron = 300`, and `deploy/README.md:161` installs *that* file to `/etc/odoo/odoo.conf`. The `0` remains only in the development configs, where the rationale is sound and documented — on Windows the watchdog's remedy is `os.kill(pid, SIGHUP)`, which is fatal rather than a graceful reload. So the dev workaround cannot ship: nothing that reaches a server carries it | Operations | `odoo.conf:60`; `.example:68` | `limit_time_real = 0` disables the runaway-request watchdog | One tenant's infinite loop permanently consumes a thread — a cross-tenant availability path. The Windows rationale is sound for dev; it must not ship | Restore on Linux (already correct there) | XS |
...
| **TST-6** | **RESOLVED 2026-08-30** — the backdates in `test_post_due_posts_only_what_has_fallen_due` are offsets from `today` via `relativedelta`, not the literals `2026-01-01`/`2026-02-01`, which were backdates only while the clock was past February 2026. **Not** `freezegun` as prescribed: Odoo ships it and exposes `odoo.tests.common.freeze_time`, but freezing suits code that reads the clock; this test needed two dates in the past and offsets say so. `freeze_time` is used for the guard instead — `test_clock_independence.py` replays the scenario at 2025-06-01, when the old literals were in the *future* and the original test would have failed | Testing | `test_loan.py:243-259` | Hardcoded 2026 backdates mixed with `context_today` | Holds only while the clock is past Feb 2026 | `freezegun` | XS |
...
| **BS-10** | **RESOLVED — closed by the conversion contract tests.** Verified 2026-08-31: `TestParseContract` asserts `parse_bs` accepts `.`, space, unpadded, whitespace-padded and Devanagari forms — exactly the separators the finding said only JS accepted — and `test_js_accepts_the_same_separator_set` pins the JS regex so the two cannot drift apart again. The second half, 'JS returns null where Python raises', is now the deliberate and documented asymmetry in `TestErrorContract`, not a defect | `bs.py:115` vs `bs_convert.js:135` | Parser divergence: JS accepts `.` and space separators, Python does not; JS returns null, Python raises |
| **BS-11** | **RESOLVED 2026-08-31** — `bsMonthLength` validated the year and then used `month` as an array index, so month 13 read one past the row and month 0 read index −1; JavaScript answers `undefined` to both rather than failing, and that `undefined` became NaN in date arithmetic far from the cause. Now refused with `BSRangeError`, matching the year check that was already there | `bs_convert.js:31-36` | `bsMonthLength` validates year but not month → `undefined` for month 13 |
| **BS-12** | **RESOLVED 2026-08-31** — `adToBs` fed its arguments to `Date.UTC`, which **normalises** instead of rejecting: `Date.UTC(2026, 8, 31)` is 1 October, so `adToBs(2026, 9, 31)` returned a real BS date for a day that never existed. Now validated by round-tripping through `Date.UTC` — a date survives unchanged only if it was real — and refused with `BSRangeError`. A control test keeps 29 February in a leap year accepted, so the check rejects impossible dates without rejecting awkward real ones | `bs_convert.js:39-42` | `adToBs` does not validate Gregorian input; `Date.UTC` normalises `2026-09-31` silently |
...
| **BS-15** | **RESOLVED 2026-08-31** — 'Replace existing' may now only replace fiscal years this wizard created. The search filtered on company and dates alone and then called `overlapping.unlink()`, so the tick deleted **every** `account.fiscal.year` overlapping the span — years a site entered by hand, and any owned by the OCA `account_fiscal_year` module — with nothing naming what was about to go, and no way to undo it from the UI. The generator has a signature (`_fiscal_year_range` builds `FY <yyyy>/<yy> (Shrawan-Ashar)`), so it can tell its own records from a site's; anything else now **stops the run with a UserError naming it**, because the two wrong outcomes are deleting a stranger's record and quietly not producing the year that was asked for. Tested in pairs: each protection check has a control proving the wizard still replaces its own years, since a fix that refused everything would pass the protection half alone. A further test asserts `_GENERATED_NAME` and `_fiscal_year_range` agree — two statements of one format | `generate_np_fiscal_year.py:88-97` | `overwrite` unlinks **any** overlapping fiscal year, no name filter, no confirmation |
...
| **COD-5** | **RESOLVED — already fixed by TST-5; the register was stale.** Verified 2026-08-30: the defensive duck-typing is gone. `'partner_id' in l._fields` and the `getattr(…, 0.0)` defaults were removed when the certificate collector was rewritten, and that method's own docstring records it: *'Both are read directly now.'* What remains is an explicit `if self.WITHHOLDING_LINE not in self.env: raise UserError(…)`, which is a precise check rather than duck-typing | `tds_certificate.py:80,92-94` | Defensive duck-typing against a pinned tree — unreachable one way, untested both |
| **COD-6** | **RESOLVED — the premise does not hold at any local site.** Verified 2026-08-30. The finding is 'broad `except Exception` **discarding tracebacks** in production paths'. All three sites in `tools/bs.py` (`:103, :117, :179`) re-raise as `UserError(… \"Underlying error: %(err)s\" …) **from exc**` — the chain is preserved *and* the underlying message is put in front of the user, which is the opposite of discarding. The only other two are annotated deliberately (`# noqa: BLE001` in `l10n_ne/verify.py`, `# pragma: no cover` in `tools/harden_database.py`). Nothing was found to fix | 4 sites | Broad `except Exception` discarding tracebacks in production paths |
| **COD-7** | **RESOLVED — was already fixed; this register was stale.** Verified 2026-08-30: repo-wide there is no `assertRaises(Exception)` in any test outside a comment *explaining* the fix, and `test_tds.py` uses `ValidationError`. Recorded rather than quietly re-ticked, because a finding that reads CONFIRMED when it is done costs the next reader the same investigation it cost this one | `test_loan.py:154`; `test_tds.py:95` | `assertRaises(Exception)` catches anything, including a typo in the test |
| **COD-8** | **RESOLVED 2026-08-30** — the app lookup and the recursive walk are now one `_delivered_names(user)` helper. They had been written out separately in two tests, each repeating the same eight lines, because neither could be simplified into a `search`: searching `ir.ui.menu` as a user does **not** apply menu group filtering, so only `load_menus` answers the question these tests ask. Test-only code, and the suite is itself the check that the extraction preserved behaviour | `test_menu_visibility.py` | Menu-tree walker written out three times in one file |
| **COD-9** | **WON'T FIX — accepted duplication, recorded 2026-08-30.** Four 'date_from precedes date_to' checks with four messages, in `account_financial_statements`, `l10n_np_tds` and `l10n_np_vat_return` (×2). Real, and left alone: the three modules share no base. `account_financial_statements` does not depend on `l10n_np`, so hosting a shared four-line validator would mean inventing a dependency between two otherwise unrelated modules to save twelve lines. The coupling costs more than the duplication | 4 modules | "date_from precedes date_to" reimplemented four times, four messages |
| **COD-10** | **RESOLVED 2026-08-30** — `nepali_datetime._days_in_month()` replaced by a public derivation: ask which day numbers `date()` accepts. The finding's *unpinned* half is stale (`requirements.txt:139` pins `1.0.8.5`, and `selftest.py` runs in the suite so a bump fails loudly), but the private call was real. **The first replacement written was wrong, and is worth recording**: deriving the length by subtracting Gregorian dates needs BS 2101-01-01 to size BS 2100-12, which the library refuses, so `month_length(2100, 12)` would have raised where it returned 30 — and BS 2100 is in scope, since `BS_MAX_YEAR` is 2100 and the fiscal-year wizard offers `current + 4`. The sweep that 'verified' it ran `MINYEAR+1..MAXYEAR-1` and skipped precisely the year that broke. The form shipped has no boundary case and is checked against the private function across the **entire** range: 1,512 months, zero mismatches, pinned by a test | `bs.py:124`; `gen_js_data.py:17` | Private `nepali_datetime` API on an unpinned library |
| **UPG-5** | **RESOLVED 2026-08-30** — the three `dict(wizard._fields['target_move'].selection).get(...)` expressions are now `t-field=\"wizard.target_move\"`, which renders a Selection's label through public API and translates it. `_fields` is ORM internals; reaching into it from a QWeb template couples a printed report to a private attribute. `t-field` was already the idiom two lines above in the same template | `financial_statements_templates.xml:42,132,221` | `wizard._fields[…].selection` — ORM internals inside QWeb |
| **UPG-6** | **RESOLVED 2026-08-30** — `noupdate=\"1\"` dropped from the province data. The record carried a self-flagged NEEDS SME CONFIRMATION *and* a guarantee that no correction would ever reach an existing database: the fix would have landed in source, passed review, shipped, and silently done nothing. Grounded in upstream convention rather than preference — `l10n_in/data/res_country_state_data.xml`, the same filename for another localisation, ships with no noupdate, because official subdivisions are reference data rather than user content | `l10n_np/data/res_country_state_data.xml:3` | Provinces `noupdate="1"` while self-flagged `NEEDS SME CONFIRMATION` — a later fix will not propagate |
| **ACC-7** | **ACCEPTED — not a defect; recorded 2026-08-31.** Counted as open, deliberately: this is a decision not to change something, which is how COD-9 and SEC-9 are also carried. The register's open figure therefore includes a handful of settled decisions as well as outstanding work, and that is better than a status word quietly reclassifying them. `SHRAWAN = 4` / `ASHAR = 3` are hard-coded in the fiscal-year wizard, and the finding itself calls this 'the one genuine exception' to the everything-is-a-record rule. It is right, and the exception is the correct call: Nepal's fiscal year runs Shrawan to Ashar by law, so promoting these to configuration would add a setting that cannot legitimately vary while creating a way to silently mis-generate every fiscal year a site ever produces. A constant that cannot change is better expressed as a constant. Recorded so it is not re-raised as an oversight | `generate_np_fiscal_year.py:12-13` | `SHRAWAN`/`ASHAR` hard-coded — the one genuine exception to "all jurisdiction values are records" |
| **SEC-9** | **WON'T FIX LOCALLY 2026-08-30 — belongs upstream; stays open.** `_eval(val, render_space)` calls `eval(val, render_space)` with a dict carrying no `__builtins__`, so Python injects the *full* builtins. The invariant making it safe is a code comment: 'not a security thread as long as the col_specs template is defined in a python module'. Traced, and it holds today. Not hardened locally, for two measured reasons. First, the obvious fix breaks the module: stripping builtins breaks `report_xlsx_helper`'s own report, whose col_specs use `len(partner.child_ids)`, and an allowlist would have to cover every expression in every downstream col_specs including `account_asset_management`'s, which cannot be enumerated from here. Second, `report_xlsx_helper` is **AGPL-3** and any local override of this 5-line static method reimplements it inside an **LGPL-3** module (LIC-1). The right fix is an upstream allowlist via `safe_eval`; the right local action is to leave the invariant documented rather than half-enforced | `report_xlsx_helper/report_xlsx_abstract.py:757,767` | `eval()` with **full builtins**; safe today (all call sites traced), invariant is a code comment |
| **SEC-10** | **RESOLVED 2026-08-30 — but not as prescribed, and the prescription was backwards.** The finding reads '`literal_eval` on a user-editable domain; `safe_eval` is house style', implying `literal_eval` is the weaker. It is the stronger: it evaluates literals and resolves no names, while `safe_eval` exists to *permit* a controlled set of names and calls. Measured against the wizard: `[('x','=',1)` gives SyntaxError, `not a domain` gives SyntaxError, `__import__('os')` gives **ValueError — refused**. Swapping would have widened the field while fixing nothing. What the measurement did show is the real defect: every rejection arrived as an unhandled traceback rather than a sentence. `account_reports_interactive/models/general_ledger_wizard.py` wraps `super()` and re-raises as a `UserError` naming the field and giving an example; **no vendored code is copied**, which matters because that module is AGPL-3 and this one LGPL-3. A test pins that code is still refused, so a later well-meaning swap to `safe_eval` cannot quietly widen it | `general_ledger_wizard.py:94` | `literal_eval` on a user-editable domain `Char`; `safe_eval` is house style |
| **SEC-11** | **RESOLVED 2026-08-30** | `l10n_ne/apply.py:46-52` | The two directories are now `sys.path.append`ed rather than `insert(0, ...)`ed. Both must be importable — `odoo` is **not** pip-installed in this venv and is found only via the repository root, and `translations` lives beside the script — but prepending put them ahead of site-packages, so a file named `polib.py` dropped into either directory would silently have replaced the installed polib for the whole process. Verified before changing it: `polib` resolves to `venv/Lib/site-packages` and `translations` resolves nowhere else, so appending still finds both |
...
| **QA-2** | **RESOLVED 2026-08-30** — the UTF-8 BOM stripped from the five remaining files, all `tests/__init__.py`. Note `tools/check_test_wiring.py` keeps reading `utf-8-sig`: that tolerance was added *because* of this finding and stays deliberately, so a reintroduced BOM breaks an assertion rather than the tooling that would otherwise report it | 6 files | UTF-8 BOM as first byte |
...
| ID | Location | Problem |
|---|---|---|
| **PKG-1** | **RESOLVED — stale; the cited line does not exist.** | `l10n_np_bs/__manifest__.py` | Verified 2026-08-30: that manifest is **28 lines** with **no `assets` key at all**, so there is no glob and nothing for it to fail to match. Same dead reference as TST-4 — the module became a compatibility shim and its assets moved to `nepali_calendar_core` | Asset glob matches nothing (see COD-3, TST-4) |
| **PKG-2** | **RESOLVED 2026-08-30** | 11 local manifests | `installable` was missing on one and `application` on seven, present as `False` on three and `True` on one — so the effective behaviour came from Odoo's defaults rather than from a decision. Both keys are now explicit in all eleven: `application=True` for `l10n_np_accounting` alone, which is the umbrella app, and `False` for the ten library modules | `installable` / `application` keys inconsistently present |
| **QA-3** | **RESOLVED 2026-08-30** | `.gitignore:53` | The negation the surrounding comment had promised is now written: `.claude/*` plus `!.claude/settings.json`. A wholesale `.claude/` ignore cannot be negated per-file — git will not re-include a file inside an excluded *directory* — so the directory is unexcluded and its contents excluded instead. Verified by exit code: `settings.local.json` ignored (0), `settings.json` not ignored (1), and git reports the latter as committable | Ignores `.claude/` wholesale, swallowing a future *shared* settings file |
| **OPS-5** | `deploy/odoo.service:37-38` | `Restart=on-failure`, `RestartSec=5s`, no `StartLimitBurst` — indefinite crash loop |
| **BS-19** | **RESOLVED — already guarded before this sweep.** | `tools/bs.py:73-74` | `to_np_digits` now tests `ch in \"0123456789\"` rather than `ch.isdigit()`, and carries a comment naming the exact trap: `str.isdigit()` is True for characters `int()` rejects, such as superscripts. Confirmed live that `int('²')` does raise and that the current code never reaches it. The finding's line reference (`bs.py:54`) is also stale | `to_np_digits` raises on exotic digit characters (`'²'.isdigit()` is True) |
| **UPG-7** | **RESOLVED — stale; the subject no longer exists.** Verified 2026-08-30: `bs_accounting_dates.py` is gone and `_BS_DATE_FIELDS` appears nowhere in any Python file. The only trace is a comment in `test_bs_calendar_integration.py:35` describing 'the retired `_BS_DATE_FIELDS` allowlist'. The mixin was removed when Bikram Sambat became global coverage in `nepali_calendar_core` rather than a per-model allowlist, which dissolved the merge hazard rather than fixing it | `_BS_DATE_FIELDS` is a plain class attribute, not `_inherit`-merged — a second module adding the mixin would silently lose one set |
